<template>
    <div>
        <h1>Оголошення</h1>
        <n-data-table :columns="listingColumns" :data="listings" :pagination="{ pageSize: 10 }" :bordered="true"
        ...
        <n-modal v-model:show="showEditListingModal" preset="dialog" title="Редагувати оголошення">
            ...
            <template #action>
                ...
                <n-button type="primary" :loading="isUpdatingListing" @click="handleUpdateListing" :disabled="!currentListing">Зберегти</n-button>
            </template>
        </n-modal>
    </div>
</template>
...
import { getAdminListings, updateAdminListing, deleteAdminListing } from '../../services/api/index';
...
const listingColumns = [
    { title: 'ID', key: 'id', minWidth: 100, sortable: true, sorter: (rowA: Listing, rowB: Listing) => rowA.id.localeCompare(rowB.id) },
    {
        title: 'Заголовок',
        key: 'title',
        minWidth: 200,
    ...
    },
    {
        ...
        sorter: (rowA: Listing, rowB: Listing) => {
            return rowA.is_agent_listing === rowB.is_agent_listing
                ? 0
                : rowA.is_agent_listing
                    ? -1
                    : 1;
        },
        render(row: Listing) {
        ...
        render(row: Listing) {
            return h(
                ...
                [
                    ...
                    h(NButton, {
                        size: 'small',
                        onClick: () => handleViewListing(row.id),
                        type: 'primary',
                        tertiary: true,
                    }, { default: () => 'Переглянути' }),
                    h(NButton, {
...
const fetchListings = async () => {
    ...
    try {
        ...
        listings.value = Array.isArray(response) ? response : (response as any).data || [];
    } catch (err) {
